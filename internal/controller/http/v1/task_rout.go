...
type taskRoutes struct {
	...
	grpcClient  *gp.GrpcClient
}

func NewTaskRouter(mux *http.ServeMux, t TaskHandlerInterface, grpcClient *gp.GrpcClient, log *logging.ZeroLogger) {
...
func (rout *taskRoutes) Get(taskId string) http.HandlerFunc { // TODO
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if validationAuthResponse.Username != task.Author.Email {
			rout.logger.Error("rout.Get() requesting information is not task author: %v; intTaskId = %d; author = %s", validationAuthResponse.Username, intTaskId, task.Author.Email)
